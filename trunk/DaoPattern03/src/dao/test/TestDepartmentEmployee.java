...
public class TestDepartmentEmployee extends TestCase {
  ...
  private boolean compareDODeep(DepartmentDTO departmentDTO1, DepartmentDTO departmentDTO2) {
    boolean ret = true;
    ...
    while (itt1.hasNext() && itt2.hasNext()) {
      ...
      compareDOShallow(employeeDTO1, employeeDTO2);
    }
  ...
  private boolean compareDODeep(EmployeeDTO employeeDTO1, EmployeeDTO employeeDTO2) {
    boolean ret = true;
    ...
    // It's getting done in compareDOShallow. May be we should cast to DO and test with it
    ret = ret && ref1.getId() == ref2.getId();

    //    compareDOShallow(ref1.getRefValue(), ref2.getRefValue());
    compareDOShallow(ref1, ref2);
  ...
  private boolean compareDOShallow(DepartmentDTO departmentDTO1, DepartmentDTO departmentDTO2) {
    boolean ret = true;
  ...
  private boolean compareDOShallow(EmployeeDTO employeeDTO1, EmployeeDTO employeeDTO2) {
    boolean ret = true;
  ...
  public void testRefOnUpdate() throws Exception {
    ...
    // XXX: Not needed anymore, it loads lazy on request
    //departmentDAO.loadEmployeeList(departmentDTOX);
    assertTrue(compareDODeep(departmentDTOa, departmentDTOX));

    DepartmentDTO departmentDTOY = (DepartmentDTO) departmentDAO.loadById(2);
    // XXX: Not needed anymore, it loads lazy on request
    //departmentDAO.loadEmployeeList(departmentDTOY);
    assertTrue(compareDODeep(departmentDTOb, departmentDTOY));
