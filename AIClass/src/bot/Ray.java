...
public class Ray implements Geometry {
  ...
  public void draw(Graphics2D g2d) {
    ...
    for (TraceBean currTraceBean : traceBeanList) {
      ...
      // 100% -> currTraceBean.dist
      //  10% -> x

      double error = g * 5 * currTraceBean.dist / 100;

      drawPrevCurrLine(g2d, prevTraceBean, currTraceBean, error);
      drawPrevCurrMark(g2d, prevTraceBean, currTraceBean, error);
  ...
  private void drawPrevCurrLine(Graphics2D g2d, //
      TraceBean prevTraceBean, TraceBean currTraceBean, double error) {

    // add some gaussian error

    double curX = calcX(currTraceBean.dist + error);
    double curY = calcY(currTraceBean.dist + error);
  ...
  private void drawPrevCurrMark(Graphics2D g2d, //
    ...
    double x = calcX(currTraceBean.dist + error);
    double y = calcY(currTraceBean.dist + error);
